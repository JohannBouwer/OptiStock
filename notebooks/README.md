...
| 11 | [Bakery Dataset](11_Bakery_Dataset.ipynb) | End-to-end demo on the real French bakery POS dataset (Kaggle): EDA, a Bayesian state-space forecast on a one-week holdout, then the ordering decision — baking the mean forecast vs. newsvendor optimization over the full posterior, with salvage economics and stochastic yield. Data is git-ignored (no open license); the notebook downloads it. |
| 12 | [Hierarchical State-Space](12_Hierarchical_StateSpace.ipynb) | Demonstrates `HierarchicalSSM`, the panel state-space forecaster with partial pooling of seasonal amplitudes and covariate coefficients: parameter shrinkage for a data-starved item (pooled vs. independent fits), joint forecasting and per-item demand distributions, and tuning the pooling strength via `HierarchicalSSMPriors` — without refitting. |
| — | [Intro to CLV Models](intro_clv_models.ipynb) | Customer lifetime value on synthetic data with a known ground truth: `ParetoNBD` for purchase frequency and silent churn, `GammaGammaSpend` for basket value, combined into a CLV leaderboard. Covers parameter recovery, holdout validation against unseen purchases, covariates (and where they actually matter), and the `*Priors` overrides. |
